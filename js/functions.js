...
// --- Google Tag Manager & PHP Form Submission ---
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', async function(e) {
      ...
      try {
        ...
        if (response.ok) {
          formStatus.style.color = 'var(--bg-green-color)'; // Verde de tu paleta
          formStatus.textContent = result.message;
          contactForm.reset();

          // Solo disparamos el evento GTM si el envío fue un éxito
          window.dataLayer = window.dataLayer || [];
          window.dataLayer.push({
            'event': 'form_submission',
            'form_id': 'contact_form',
            'form_name': 'Contact Discava',
            'page_location': window.location.href
          });
        } else {
          formStatus.style.color = '#ff6b6b'; // Rojo de error
