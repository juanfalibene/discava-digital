...
async function showDiscoData(dataMaster, typeOfVersion) {
  ...
  ratingLi.textContent = `Rating: ${dataMaster.community.rating.average}`;
  const haveWantLi = document.createElement("li");
  haveWantLi.textContent = `Have: ${dataMaster.community.have} / Want: ${dataMaster.community.want}`;
  const copiesPriceLi = document.createElement("li");
  copiesPriceLi.textContent = `Copies for sale: ${dataMaster.num_for_sale} from ${dataMaster.lowest_price}`;
  ...
  for (const track of dataMaster.tracklist) {
    ...
    // Antes del bucle de tracklist, crea un contenedor para los botones de video
    const videoContainer = document.createElement("div");
    singleTracklistUL.appendChild(videoContainer);
    ...
    for (const track of dataMaster.tracklist) {
      ...
      // buscar videos según title
      if (dataMaster.hasOwnProperty("videos")) {
        const videoLinks = searchVideos(track, dataMaster.videos);

        if (videoLinks.length > 0) {
          for (const videoLink of videoLinks) {
            videoContainer.appendChild(videoLink); // Adjunta cada botón al contenedor de videos
          }
        }
      }
    }
